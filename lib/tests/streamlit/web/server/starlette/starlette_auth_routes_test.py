...
class TestGetProviderByState:
    ...
    def test_handles_malformed_cache_keys(self) -> None:
        ...
        # Add a valid key
        _STARLETTE_AUTH_CACHE._cache["_state_github_xyz789"] = (
            {"some": "data"},
            future_exp,
        )

        # Should still find the valid key
        assert _get_provider_by_state("xyz789") == "github"
        # Should return None for unmatched state
        assert _get_provider_by_state("abc123") is None
